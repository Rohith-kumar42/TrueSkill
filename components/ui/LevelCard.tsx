...
export function LevelCard({ program }: { program: Program }) {
  return (
    <Link href={`/programs/${program.slug}`} className="group glass-surface chess-corners block rounded-lg p-7 transition duration-200 hover:-translate-y-1 hover:border-gold/50 hover:shadow-gold active:scale-[0.98]">
      ...
      <span className="mt-6 inline-flex items-center gap-2 font-cinzel text-sm text-gold">
        See what you&apos;ll learn <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
      </span>
    </Link>
  );
}
